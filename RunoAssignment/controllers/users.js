...
const loginUser = async (req,res)=>{
    ...
    try{
        const user = await userModel.findOne({phone_number,hashedPass})
        if(!user){
...
const signupUser = async (req, res) => {
    ...
    const { name, phone_number, email,age ,pincode, password,aadhar } = req.body
      const hashedPass = await bcrypt.hash(password, 5)
        try {
          const newUser = await userModel.create({  name, phone_number, email,age ,pincode,aadhar, password: hashedPass, })
          res.redirect('/login')
